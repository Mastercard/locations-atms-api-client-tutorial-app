...
6. Update the properties found in `src/main/java/com/mastercard/locations/atms/ApiService.java`.
...
6. Run `mvn clean generate-sources` from the root of the project directory.
    * When compile is run, the [OpenAPI Generator plugin](#integrating-with-openapi-generator) will generate the sources for connecting to the Atm Locations API.
        ...
        <plugin>
            ...
            <executions>
                <execution>
                    ...
                    <configuration>
                        <inputSpec>${project.basedir}/src/main/resources/atms-locations-api-spec.yaml</inputSpec>
...
These are the parameters used for the Atm Locations API. All parameters are optional.
...
### Atm Search <a name="atms"></a>
> Get a paginated list of atms that fit the search criteria. 

| URL       | Method        | Parameters        | Request Model | Response model|
|-----------|---------------|-------------------|---------------|---------------|
|/searches| POST     | offset, limit, distance, distance_unit     | AtmSearch    | Atms |
...
| ----------------------------- | ----------------------------------------------------------------- |
| [Atms Search](https://developer.mastercard.com/locations/documentation/api-reference/#api)                 | Retrieve atm detail for a given geo location |
